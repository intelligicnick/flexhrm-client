...
describe('GeM listing status parsing', () => {
  ...
  it('parses qualified + bid award as bid-award stage, not won', () => {
    ...
    const card = document.querySelector('.card') as HTMLElement;
    const status = parseGemCardStatus(card);
    expect(status.status).toBe('bid_awarded');
    expect(status.outcome).toBe('Bid Awarded');
  ...
  it('parses brown bid award stage as bid not awarded', () => {
    const bidNotAwarded = [
      'Technical Evaluation (completed)',
      'Financial Evaluation (completed)',
      'Bid Award (not awarded)',
    ];
    expect(
      deriveStatusFromGemProgress('Bid Award', 'Qualified', bidNotAwarded, true),
    ).toBe('bid_not_awarded');
  });
  ...
  it('parses Bid / RA Award with View BID Results as bid awarded', () => {
    const cardText = `
...
`;
  ...
  it('parses Bid / RA Award in progress as qualified', () => {
    const cardText = `
BID NO GEM/2025/B/6746241
Status:
Bid / RA Award
...
`;
  ...
  it('detects green bid award colour from live GeM DOM on sync', () => {
    document.body.innerHTML = `
      <div class="card">
        ...
        <span id="bid-award" style="color: rgb(34, 150, 34)">BID AWARD</span>
      </div>`;
    const card = document.querySelector('.card') as HTMLElement;
    const status = parseGemCardStatus(card);
    expect(status.status).toBe('bid_awarded');
    expect(status.outcome).toBe('Bid Awarded');
    expect(status.gemCurrentStage.toLowerCase()).toContain('bid award (completed)');
  });

  it('detects brown bid award colour as bid not awarded on sync', () => {
    document.body.innerHTML = `
      <div class="card">
        Status: Bid Award
        Technical Status Qualified
        <span>TECHNICAL EVALUATION</span>
        <span>FINANCIAL EVALUATION</span>
        <span style="color: rgb(180, 120, 60)">BID AWARD</span>
      </div>`;
    const card = document.querySelector('.card') as HTMLElement;
    const status = parseGemCardStatus(card);
    expect(status.status).toBe('bid_not_awarded');
    expect(status.outcome).toBe('Bid Not Awarded');
  });
});
