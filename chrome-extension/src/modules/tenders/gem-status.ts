...
function isBrownColor(color: string): boolean {
  const rgb = parseRgb(color);
  if (!rgb) return /brown|amber|warning|not[- ]?award/i.test(color);
  const [r, g, b] = rgb;
  return r > 90 && g > 50 && g < r + 30 && b < g - 10;
}

function detectStageColor(el: HTMLElement): 'blue' | 'green' | 'grey' | 'brown' | '' {
  ...
  for (let depth = 0; depth < 8 && node; depth += 1) {
    ...
    if (isBrownColor(style.backgroundColor) || isBrownColor(style.color)) return 'brown';
...
type StageState = 'in progress' | 'completed' | 'pending' | 'not awarded';

function inferBidAwardStateFromCard(cardText: string, processStatus: string): StageState {
  if (!normalizeBidAwardHeader(processStatus)) return 'pending';
  if (/view\s+bid\s+results/i.test(cardText)) return 'completed';
  if (/not\s+(?:selected|awarded|l1)|unsuccessful|not\s+won|lost\s+bid/i.test(cardText)) {
    return 'not awarded';
  }
  return 'in progress';
}
...
export function parseProgressStages(
  card: HTMLElement,
  processStatus: string,
  textOverride?: string,
): string[] {
  const cardText = textOverride?.trim() || readCardText(card);
  const cardTextUpper = cardText.toUpperCase();
  const lines: string[] = [];
  const stageDefs =
    cardTextUpper.includes('TECHNICAL EVALUATION') || cardTextUpper.includes('FINANCIAL EVALUATION')
      ? STAGE_DEFS
      : cardTextUpper.includes('EVALUATION') && cardTextUpper.includes('BID AWARD')
        ? ([
  ...
  for (const stage of stageDefs) {
    if (!cardTextUpper.includes(stage.key)) continue;

    const el = findStageElement(card, stage.key);
    const color = el ? detectStageColor(el) : '';
    let state: StageState;
    if (color === 'green') state = 'completed';
    else if (color === 'brown') state = 'not awarded';
    else if (color === 'blue') state = 'in progress';
    else if (color === 'grey') state = 'pending';
    else if (stage.key === 'BID AWARD') {
      state = inferBidAwardStateFromCard(cardText, processStatus);
    } else {
      state = inferStageStateFromHeader(stage.label, processStatus);
    }
...
export function parseGemCardStatus(card: HTMLElement, textOverride?: string): GemCardStatus {
  const text = textOverride?.trim() ? textOverride : readCardText(card);
  ...
  const stageLines = parseProgressStages(card, processStatus, text);
  ...
  if (technicalStatus && mapTechnicalResult(technicalStatus).status === 'disqualified') {
    ...
    outcome = bidAwardOutcome(bidAward, selfAwarded);
...
export function applyCardStatusToTender(
  ...
  const bidNo = tender.bidNo.toUpperCase();
  const domCard = expandCardForStatusParsing(card, bidNo);
  const scopedText = readStatusScopeText(card, bidNo);
  const statusFields = parseGemCardStatus(domCard, scopedText);
  return {
    ...tender,
