...
const SAVE_TARGET_PATHS: Record<SaveTargetType, string> = {
...
export async function saveCapture(
  ...
  const path = SAVE_TARGET_PATHS[draft.saveTarget];
...
export async function checkTenderDuplicates(
  ...
  return request(config, '/tenders/duplicate-check', {
    method: 'POST',
...
export async function checkContractDuplicates(
  ...
  return request(config, '/contracts/duplicate-check', {
    method: 'POST',
    body: JSON.stringify({ contractKeys }),
  });
}
  ...
  try {
    ...
    if (!navigator.onLine || (err instanceof FlexHRMApiError && err.status === undefined)) {
      const queueItem: QueuedRecord = {
        id: draft.id,
        endpoint: SAVE_TARGET_PATHS[draft.saveTarget],
        method: 'POST',
