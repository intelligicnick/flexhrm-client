...
export default function EmployeeTable({
  ...
  const stickyColumnsContainerRef = useRef<HTMLDivElement | null>(null);

  const setStickyColumnsContainer = useCallback((node: HTMLDivElement | null) => {
    stickyColumnsContainerRef.current = node;
    if (node) {
      node.setAttribute("data-employee-sticky-columns", "true");
    }
  }, []);

  const handleStickyColumnsChange = useCallback((enabled: boolean) => {
    stickyColumnsContainerRef.current?.setAttribute("data-employee-sticky-columns", enabled ? "true" : "false");
  }, []);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden" id="employee-table-component">
      ...
      {/* Main Datagrid */}
      <div
        ref={setStickyColumnsContainer}
        className="border border-slate-150 rounded-lg shadow-inner overflow-hidden"
      >
        <div className="flex items-center gap-1.5 px-2.5 py-1 border-b border-slate-100 bg-white">
          <Switch
            defaultChecked
            onCheckedChange={handleStickyColumnsChange}
              ...
              filteredEmployees.map((emp) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className={`employee-sticky-col-0 ${stickyCellClassName} p-3 w-[48px] min-w-[48px] max-w-[48px] text-center border-r border-slate-200`}>
                      <input
                        type="checkbox"
                        checked={isSelected}
                    ...
                    <td className={`employee-sticky-col-1 ${stickyCellClassName} p-3 w-[60px] min-w-[60px] max-w-[60px] text-center font-bold text-slate-450 border-r border-slate-200`}>
                      {emp.srNo}
                    </td>

                    <td 
                      onClick={() => setViewEmployee(emp)}
                      className={`employee-sticky-col-2 ${stickyCellClassName} p-3 w-[110px] min-w-[110px] max-w-[110px] font-semibold text-blue-600 border-r border-slate-250 cursor-pointer hover:underline hover:text-blue-800 overflow-hidden`}
                      title="Click to view full employee dossier"
                    >
                      {emp.employeeCode}
                    </td>

                    <td className={`employee-sticky-col-3 ${stickyCellClassName} p-3 w-[220px] min-w-[220px] max-w-[220px] font-bold text-slate-900 truncate overflow-hidden border-r border-slate-300`} title={emp.nameAsPerAadhar}>
                      {emp.nameAsPerAadhar}
                    </td>
