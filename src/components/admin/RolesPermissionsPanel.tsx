...
import {
  ALL_OBSERVER_MODULE_IDS,
  DEFAULT_OBSERVER_ALLOWED_MODULES,
  OBSERVER_MODULE_ROWS,
...
export default function RolesPermissionsPanel() {
  ...
  const grantObserverAdminRolePreset = () => {
    ...
    setRoleUiInput((prev) => ({
      ...
      observer: { allowedModules: [...DEFAULT_OBSERVER_ALLOWED_MODULES] },
    }));
  };

  const isObserverModuleChecked = (moduleId: string) => {
    const allowed = roleUiInput.observer?.allowedModules;
    if (allowed === undefined) return true;
    if (allowed.length === 0) return false;
    return allowed.includes(moduleId);
  };

  const allObserverModulesSelected = OBSERVER_MODULE_ROWS.every((mod) =>
    isObserverModuleChecked(mod.id),
  );
  const someObserverModulesSelected = OBSERVER_MODULE_ROWS.some((mod) =>
    isObserverModuleChecked(mod.id),
  );

  const selectAllObserverModules = () => {
    setRoleUiInput((prev) => ({
      ...prev,
      observer: { ...prev.observer, allowedModules: undefined },
    }));
  };

  const clearAllObserverModules = () => {
    setRoleUiInput((prev) => ({
      ...prev,
      observer: { ...prev.observer, allowedModules: [] },
    }));
  };

  const toggleObserverModule = (moduleId: string) => {
    setRoleUiInput((prev) => {
      const allIds = [...ALL_OBSERVER_MODULE_IDS];
      const current =
        prev.observer?.allowedModules !== undefined
          ? prev.observer.allowedModules
          : allIds;
      ...
      return {
        ...
        observer: {
          ...prev.observer,
          allowedModules: next.length === allIds.length ? undefined : next,
        },
      };
    });
  };
              ...
              <div className="p-5 space-y-4 flex-1">
                ...
                <div className="rounded-xl border border-orange-200 bg-orange-50/40 p-4 space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      <h5 className="text-[11px] font-black text-orange-800 uppercase tracking-wider">
                        Observer Admin modules
                      </h5>
                      <p className="text-[10px] text-orange-700 mt-1">
                        Unchecked modules are hidden in the Observer Admin mobile app. Users still need View permission on the backing module.
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2 shrink-0">
                      <button
                        type="button"
                        onClick={selectAllObserverModules}
                        className="px-2.5 py-1 bg-white border border-orange-200 rounded-lg text-[10px] font-bold text-orange-800 hover:bg-orange-100 transition cursor-pointer"
                      >
                        Select all
                      </button>
                      <button
                        type="button"
                        onClick={clearAllObserverModules}
                  ...
                  <label className="flex items-center gap-2 text-[11px] font-bold text-orange-900 cursor-pointer border-b border-orange-100 pb-2">
                    <input
                      type="checkbox"
                      checked={allObserverModulesSelected}
                      ref={(el) => {
                        if (el) {
                          el.indeterminate =
                            someObserverModulesSelected && !allObserverModulesSelected;
                        }
                      }}
                      onChange={(e) =>
                        e.target.checked ? selectAllObserverModules() : clearAllObserverModules()
                      }
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                    {OBSERVER_MODULE_ROWS.map((mod) => (
                      <label
                        key={mod.id}
                        className="flex items-center gap-2 text-[11px] font-semibold text-slate-700 cursor-pointer rounded-lg border border-orange-100 bg-white px-2.5 py-2"
                      >
                        <input
                          type="checkbox"
                          checked={isObserverModuleChecked(mod.id)}
                          onChange={() => toggleObserverModule(mod.id)}
